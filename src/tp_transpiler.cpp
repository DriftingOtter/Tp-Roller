...
std::string regex_tp_heading_to_html_heading(std::string current_line) {
  ...
  std::regex h3_pattern(
      R"((?!(~~~|\[url=\"\"|\[vid=\"\"|\[aud=\"\"|\[img=\"\"|\[|\])).*\[\[\[([^[\]]*)\]\]\])");
  std::regex h4_pattern(
      R"((?!(~~~|\[url=\"\"|\[vid=\"\"|\[aud=\"\"|\[img=\"\"|\[|\])).*\[\[\[\[([^[\]]*)\]\]\]\])");
  std::regex h5_pattern(
      R"((?!(~~~|\[url=\"\"|\[vid=\"\"|\[aud=\"\"|\[img=\"\"|\[|\])).*\[\[\[\[\[([^[\]]*)\]\]\]\]\])");
  std::regex h6_pattern(
      R"((?!(~~~|\[url=\"\"|\[vid=\"\"|\[aud=\"\"|\[img=\"\"|\[|\])).*\[\[\[\[\[\[([^[\]]*)\]\]\]\]\]\])");
...
std::string regex_tp_unordered_list_to_html(std::string current_line,
...
std::string regex_tp_numbered_list_to_html(std::string current_line,
                                           bool& in_numbered_list) {
  std::string html;
...
std::string regex_tp_alphabetical_list_to_html(std::string current_line,
                                               bool& in_alphabetical_list) {
  std::string html;
...
std::string regex_tp_preamble_to_html_metadata(std::string current_line) {
  ...
  if (std::regex_search(current_line, match, title_pattern)) {
    ...
    std::string lang_attribute =
        match[1].str().substr(0, 3);  // Take first three letters
    std::transform(lang_attribute.begin(), lang_attribute.end(),
                   lang_attribute.begin(), ::tolower);  // Convert to lowercase
...
std::string process_lists(std::string current_line,
                          bool& in_unordered_list,
                          bool& in_numbered_list,
                          bool& in_alphabetical_list) {
  ...
  list_html =
      regex_tp_alphabetical_list_to_html(current_line, in_alphabetical_list);
  if (!list_html.empty()) {
    return list_html;
  }
...
std::string process_table(std::string current_line, bool& in_table) {
  std::string table_html;
  table_html = regex_tp_table_column_to_html_table_tag(
      current_line);  // Check if line starts or ends a table
  if (!table_html.empty()) {
    return table_html;
  }
...
std::string process_other_html_elements(std::string current_line) {
...
std::string transpile_to_html(std::string& current_line) {
  ...
  result = process_preamble(current_line, in_preamble, accumulated_preamble);
  if (!result.empty()) {
    return result;
  }
  ...
  result = process_table(current_line, in_table);
  if (!result.empty()) {
    return result;
  }
